<div class="alert mycolor1" role="alert">항공사</div> <!-- 사용자헤더 -->
	<script>
		function find_text()
		{
			if (!form1.text1.value)					// 값이 없는 경우, 전체 자료
				form1.action="/~team19/air/lists";
			else									// 값이 있는 경우, text1 값 전달
				form1.action="/~team19/air/lists/text1/" + form1.text1.value;
			form1.submit();
		}
	</script>
	...
	?>
				<div class="col-9" align="right">	<!--추가버튼 부분-->
					 <a href="/~team19/air/add<?=$tmp; ?>" class="btn btn-sm mycolor1">추가</a>
				</div>
			</div>
		</form>
...
<tr>
    <td><?=$no; ?></td>
    <td><a href="/~team19/air/view/no/<?=$no?><?=$tmp?>"><?=$row->name; ?></a></td>
</tr>
<?
    }
?>
