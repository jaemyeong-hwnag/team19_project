<?
    class Kind_m extends CI_Model							// 모델 클래스 선언
    {
        public function getlist($text1,$start,$limit) // 해당 페이지 자료 읽기
        {
			if (!$text1)
				$sql="select * from kind order by name"; // select문 정의, 전체자료
			else
				$sql="select * from kind where name like '%$text1%' order by name limit $start,$limit";
		...
		{
			$sql="select * from kind where no=$no";
			return $this->db->query($sql)->row();
		}
		function deleterow($no)	// 삭제
		{
			$sql="delete from kind where no=$no";
			return $this->db->query($sql);
		}
		function insertrow($row)	// 추가
		{
			return $this->db->insert("kind",$row);
		}
		function updaterow($row, $no)	// 수정
		{
			$where=array("no"=>$no);
			return $this->db->update("kind", $row, $where);
		}
    }
?>