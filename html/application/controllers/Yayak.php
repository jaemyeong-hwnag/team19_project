<?	// 클래스이름은 파일이름과 같아야 하며 첫 글자는 반드시 대문자여야 한다.
    class Yayak extends CI_Controller {				// yeyag클래스 선언
        function __construct()							// 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();					// 데이터베이스 연결
            $this->load->model("yayak_m");				// 모델 yayak_m 연결
        ...
        {	
			...
			$config["total_rows"] = $this->yayak_m->rowcount($text1, $member_no); // 전체 레코드개수 구하기
            ...
            $data["list"] = $this->yayak_m->getlist($text1,$start,$limit, $member_no);	// 목록읽기, 해당페이지 자료읽기

            $this->load->view("main_header");			// 상단출력(메뉴)
			// $this->load->view("yayak_title");
            $this->load->view("yayak_list",$data);		// yayak_list에 자료전달
		...
		public function del()
		{
			...
			redirect("/~team19/yayak/lists/" . $text1 . $page);	// 목록화면으로 돌아가기
		}
		public function add()
		{
			$uri_array=$this->uri->uri_to_assoc(3);
			...
			else										// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$result = $this->yayak_m->insertrow($data); // insertrow함수 호출, 데이터저장

				redirect("/~team19/admin/yayak/lists/" . $text1 . $page);		// 목록화면으로 이동
			}
		}
		public function edit()
		{
			...
			else										// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$result = $this->yayak_m->updaterow($data,$no); // insertrow함수 호출, 데이터저장
		...
		{
			$config['upload_path'] = "./yayak_img";	// 저장할 경로
