<?	// 클래스이름은 파일이름과 같아야 하며 첫 글자는 반드시 대문자여야 한다.
    class Yayak extends CI_Controller {				// yeyag클래스 선언
        ...
        public function lists()
        {
			...
			$config["total_rows"] = $this->yayak_m->rowcount($text1); // 전체 레코드개수 구하기
            ...
            $data["list"] = $this->yayak_m->getlist($text1,$start,$limit);	// 목록읽기, 해당페이지 자료읽기
		...
		public function del()
		{
			...
			redirect("/~team19/admin/yayak/lists/" . $text1 . $page);	// 목록화면으로 돌아가기
		}
		public function add()
		{
			$uri_array=$this->uri->uri_to_assoc(3);
