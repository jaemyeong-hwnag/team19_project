<?	// 클래스이름은 파일이름과 같아야 하며 첫 글자는 반드시 대문자여야 한다.
    class Country extends CI_Controller {				// country클래스 선언
        function __construct()							// 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();					// 데이터베이스 연결
            $this->load->model("country_m");				// 모델 country_m 연결
        ...
        public function lists()
        {
            ...
            $data["list"] = $this->country_m->getlist($text1,$start,$limit);	// 목록읽기, 해당페이지 자료읽기

            $this->load->view("admin/main_header2");			// 상단출력(메뉴)
			//echo "ok";
			// $this->load->view("country_title");
            $this->load->view("admin/country_list",$data);		// country_list에 자료전달
		...
		public function del()
		{
			...
			// $no=$this->uri->segment(4);					// URI: /country/del/no/1
			$this->country_m->deleterow($no);
			redirect("/~team19/country/lists/" . $text1 . $page);	// 목록화면으로 돌아가기
		}
		public function add()
		{
			$uri_array=$this->uri->uri_to_assoc(3);
			...
			else										// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$result = $this->country_m->insertrow($data); // insertrow함수 호출, 데이터저장

				redirect("/~team19/country/lists/" . $text1 . $page);		// 목록화면으로 이동
			}
		}
		public function edit()
		{
			...
			else										// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$result = $this->country_m->updaterow($data,$no); // insertrow함수 호출, 데이터저장
