<?	// 클래스이름은 파일이름과 같아야 하며 첫 글자는 반드시 대문자여야 한다.
    class Yeyag extends CI_Controller {				// yeyag클래스 선언
        ...
        public function lists()
        {
			...
			$base_url = "/~team19".$base_url;	// sale계정인 경우
		...
		public function del()
		{
			...
			redirect("/~team19/yeyag/lists/" . $text1 . $page);	// 목록화면으로 돌아가기
		}
		public function add()
		{
			$uri_array=$this->uri->uri_to_assoc(4);
			...
			$this->form_validation->set_rules("tour_no","상품명","required");
			//$this->form_validation->set_rules("tour_no","상품종류","required|max_length[50]");
			...
			else										// 입력화면의 저장버튼 클릭한 경우
			{
				...
				redirect("/~team19/yeyag/lists/" . $text1 . $page);		// 목록화면으로 이동
			}
		}
